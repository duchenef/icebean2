...
/*   setup for each search box view */ 
 ...
 //unformatted default - subcoding either ignored or set to --
    ...
    $j("#commonFacetList").html("<label>Limit Results by:</label>" +buildSelectLimitList("commonfacetlist",facetList, facetList) );
    $j('#commonbox').autocomplete(	  {
          source: autoSubjectCommon, 
          minLength: 1,
   		 select: function(event, ui) {
              $j('#commonXtra').html("FAST ID " + ui.item.idroot + " Type "+ getTypeFromTag(ui.item.tag));
              selected = true;
          } //end select
 ...
 //marcBreaker format 
    ...
    $j("#breakerFacetList").html("<label>Limit Results by:</label>" +buildSelectLimitList("breakerfacetlist",facetList));
 ...
 //connexion format     
    ...
    $j("#connexFacetList").html("<label>Limit Results by:</label>" +buildSelectLimitList("connexfacetlist",facetList));
    $j('#connexbox').autocomplete(	  {
          source: autoSubjectConnex, 
          minLength: 1,
   		 select: function(event, ui) {
              $j('#connexXtra').html((ui.item.tag+500) + "  " + ui.item.indicator + "7 " );
              selected = true;
          }
          , open: function(event,ui) {
             selected = false;  // reopened menu, reset selected
...
function formatSuggest(ul, item  ) {
   ...
   if(item.type=="alt")
	...
	return $j( "<li></li>" )
		.data( "item.autocomplete", item ).append( "<a><span style='font-size: 80%'>" + retValue + "</span></a>" ).appendTo( ul );
